...
# 22 取值运算符
下面这张图肯定是执行出错的，因为没有地址是1的变量分配，只是能理解汇编就行，取值运算符
![[Pasted image 20240310143335.png]]
...
数组作为参数时，应该传递数组的长度
![[Pasted image 20240310151222.png]]


方法二：
可以使用指针传递数组首地址
![[Pasted image 20240310150929.png]]
...
# 27函数调用约定
![[Pasted image 20240310162323.png]]

# 28 函数指针
前提是知道函数有几个参数[[bah-host碰撞-pspy+shellinabox]]
下图只是举个例子，肯定是不能执行的，应为赋值10不是一个准确的函数地址
这种方法可以用来做3环的对抗，比如绕过上层函数地址去内层函数直接执行，防止别人在上层函数下断点
![[Pasted image 20240310163656.png]]
函数指针不能做运算
# 30 预处理之宏定义
