...
vs中属性--C/C++--所有选项--支持仅对我调试的代码--改成否：：就能写出和VC6一样简洁的代码。否则不知为何会多出一个`call  ConsoleA.__CHeckForDebuggerJustMycode`

vs属性--配置属性--常规--平台工具集--要选到xp：：这样发给xp系统才能兼容运行
vs中属性--》配置属性--》常规--》字符集--》使用多字节字符集

vs中属性--》C/C++--》所有选项--》SDL检查（否）、安全检查（禁用）、基于运行时的检查（未初始化的变量）、运行库（MTd才是静态编译）预编译头--否

vs中属性--链接器--》所有选项--》增量链接（否）、数据执行保护（关）、随机基址(编译dll就不用关)--否

vs中属性--链接器--生成调试信息--》/DEBUGFULL（这样才能看到调试信息，否则调试的时候所有call都没有名子）
...
花指令是防止静态分析IDA的，设计思路是构造恒成立的EIP跳转，里面插无效代码（防静态分析，防不住动态调试，防字符串搜索）。上面图片就是构造了jmp中间写入了数据，但是反编译工具逐条解释遇到e8当成了call，接着就把后面xxx指令当成了call的地址，反编译就不能F5了。手动去除花指令，看他的半个跳转的位置，然后请在数据窗口中修改跳转目地的前面多出来的数据nop（还可以右键--分析--从下次代码分析的时候设置为byte--再分析代码--就会把这一行变成数据）。
