...
* Main函数真的是最早执行的函数吗？
先写一段代码：zhi'xie'ge
在实验中写一篇C++语言的代码cpp，定义一个类和public方法，构造函数在main函数之前，并定义类的全局变量，然后去OD中从EP开始跟踪观察会发现一个叫_Initterm的函数call，这个就是C语言用来初始化全局变量的函数。是优先于main函数执行的。
...
在AddressOfCallBacks保存的就是![image.png](https://img-blog.csdnimg.cn/img_convert/3be0a04e8c2b3459b6003a2a02eb2f5c.png)

里面的函数地址。顺序存储

遇到这种情况将程序放到OD，那么TLS的函数会直接运行起来，然后再停在OEP的位置，这就是TLS反调试。此时再OD菜单栏--选项--调试设置--事件--设置第一次暂停于--系统断点。再用OD插件StrongOD--OPtion--勾选Break On Tls就可以直接停在Tls的回调函数上。
...
花指令--不仅能由jcc指令来实现，还能由call,retn指令来实现。
