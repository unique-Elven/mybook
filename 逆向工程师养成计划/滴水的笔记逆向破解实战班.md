...
花指令是防止静态分析IDA的，设计思路是构造恒成立的EIP跳转，里面插无效代码（防静态分析，防不住动态调试，防字符串搜索）。上面图片就是构造了jmp中间写入了数据，但是反编译工具逐条解释遇到e8当成了call，接着就把后面xxx指令当成了call的地址，反编译就不能F5了。手动去除花指令，看他的半个跳转的位置，然后请在数据窗口中修改跳转目地的前面多出来的数据nop（还可以右键--分析--从下次代码分析的时候设置为byte--再分析代码--就会把这一行变成数据）。易语言插件--去除花指令MainFDlg。
