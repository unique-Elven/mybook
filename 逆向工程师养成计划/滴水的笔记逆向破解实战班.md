...
>如果NtQueryInformationProcess的结果全局变量isDebug不等于0, 则说明发现被调试了，就会把esi的0值赋值给edi，EDI原本是存储的下一个TLS函数的地址，如果没被发现调试，那就给AddressOfCallBacks的后面接着传入下一个TLS函数的地址，让程序继续执行TLS的回调函数。
